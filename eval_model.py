...
from lib.metrics import compute_video_ar, compute_video_psnr_and_ssim
...
def do_eval(test, info, seed):
    ...
    checkpoints = [Path(test['out_dir'], 'experiments', 'weights', test['checkpoint'])]
    ...
    out_dir = Path(test['out_dir'], 'experiments', 'results', exper_name, \
    ...
    for c_idx,c in enumerate(checkpoints):
        ...
        print('\tModel doing {} rollouts'.format(model.module.stochastic_samples))
